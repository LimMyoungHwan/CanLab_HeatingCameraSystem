...
| 항목 | 계획 | 실제 | 이유 |
|---|---|---|---|
| 챔버 온도 출처 | 캡처 스텝의 목표 온도 | **촬영 직전 PLC 실측값** (`GetCurrentTemperatureAsync`) | 캡처 스텝과 챔버 스텝이 분리되어 목표를 알 수 없음. 한때 기록 조건이 남긴 값을 재사용했으나, 기록 조건이 모두 0인 레시피에서 저장이 통째로 멈춰 되돌림 |
| 온도 → 코드 | 정확 일치 | **무조건 최근접 스냅**(오차 한계 없음) | 실측값이라 39.98℃처럼 들어옴. 한때 `TemperatureTolerance` 밖이면 `RCP-005`로 막았는데, 챔버 미제어 상온 촬영(실측 26.6℃)에서 저장이 통째로 스킵돼 되돌림 — 폴더명은 검증이 아니라 규칙 |
...
| `.y16` 저장 | 규칙 저장과 병행 | **규칙 저장 시 배치당 1장만** | 장마다 쓰면 디스크·쓰기 시간이 두 배. 방사 측정 원본은 `.raw`가 담당 |
| 캡처 결과 | 장마다 1건 | **규칙 저장 시 배치당 1건** | 장마다 JPEG을 실으면 100장 × 카메라수 만큼의 미리보기가 Master 메모리에 상주 |
