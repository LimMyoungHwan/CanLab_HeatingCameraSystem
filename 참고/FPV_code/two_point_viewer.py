...
class ThermalUI:
    ...
    def _render_current(self):
        ...
        display_w, display_h = self.canvas_shape
        if img.size != (display_w, display_h):
            resample_nearest = getattr(getattr(Image, "Resampling", Image), "NEAREST")
            img = img.resize((display_w, display_h), resample_nearest)

        self.single_canvas.config(width=display_w, height=display_h)
...
class TECLESS(ThermalUI):
    ...
    def merge_dead_pixels_coords(self, DP_mask_collected):
        ...
        if os.path.isfile(lynred_csv_path):
            ...
            lynred_mask = np.zeros((240, 320), dtype=bool)
