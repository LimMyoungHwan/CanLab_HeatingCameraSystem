...
class custom:
    def __init__(self, root):
        ...
        self._histogram_render_pending = False
        self.bias_dict = {}     
        self.root = root
        ...
        self.target_camera_fps = 30.0
        self.camera_frame_interval = 1.0 / self.target_camera_fps
        ...
        # 한글 텍스트가 있는 버튼에서 글자 잘림이 생기지 않도록 한글 폰트 기준으로 스타일 지정
        self.button_style = ttk.Style(self.root)
        self.button_style.configure("TButton", font=("Malgun Gothic", 10), padding=(9, 5))
        self.button_style.configure("Bias.TButton", font=("Malgun Gothic", 9), padding=(9, 5))
        ...
        self.cam_button = ttk.Button(self.control_frame, text="Connect")
        self.cam_button.configure(
            command=self._make_button_guarded_command(self.cam_button, self.check_frame_num)
        )
        self.cam_button.grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.cam_disconnect_button = ttk.Button(self.control_frame, text="Disconnect")
        self.cam_disconnect_button.configure(
            command=self._make_button_guarded_command(
        ...
        self.port_refresh_button = ttk.Button(self.control_frame, text="UPDATE")
        self.port_refresh_button.configure(
            command=self._make_button_guarded_command(
                self.port_refresh_button, self.refresh_available_ports
            )
        )
        self.port_refresh_button.grid(row=0, column=2, padx=5, pady=5, sticky="w")
        self.shutter_button = ttk.Button(self.control_frame, text="Shutter")
        self.shutter_button.configure(
            command=self._make_button_guarded_command(self.shutter_button, self.shutter_switch)
        )
        self.shutter_button.grid(row=1, column=2, padx=5, pady=5, sticky="w")
        self.save_button = ttk.Button(self.control_frame, text="저 장")
        self.save_button.configure(
            command=self._make_button_guarded_command(self.save_button, self.save_rawfile)
        )
        self.save_button.grid(row=8, column=1, padx=5, pady=5, sticky="w")
        self.validation_button = ttk.Button(self.control_frame, text="검 증")
        self.validation_button.configure(
            command=self._make_button_guarded_command(
                self.validation_button, self.validation_check
            )
        )
        self.validation_button.grid(row=8, column=0, padx=5, pady=5, sticky="w")
        self.set_save_dir_button = ttk.Button(self.control_frame, text="저장 폴더 변경")
        self.set_save_dir_button.configure(
            command=self._make_button_guarded_command(
        ...
        self.bias_button = ttk.Button(
            self.control_frame, text="자동 바이어스", style="Bias.TButton"
        )
        self.bias_button.configure(
            command=self._make_button_guarded_command(
                self.bias_button,
                lambda: self.auto_bias_setting(self.connected_cameras),
                in_progress_check=lambda: self.auto_bias_running,
            )
        )
        self.bias_button.grid(row=5, column=0, padx=5, pady=5, sticky="w")
        self.bias_read_button = ttk.Button(self.control_frame, text="개별 설정값 불러오기")
        self.bias_read_button.configure(
            command=self._make_button_guarded_command(
                self.bias_read_button, lambda: self.bias_read(self.C_index)
            )
        )
        self.bias_read_button.grid(row=5, column=1, padx=5, pady=5, sticky="w")
        self.all_bias_read_button = ttk.Button(self.control_frame, text="전체 설정값 불러오기")
        self.all_bias_read_button.configure(
            command=self._make_button_guarded_command(
                self.all_bias_read_button, self.all_bias_read
            )
        )
        self.all_bias_read_button.grid(row=5, column=2, padx=5, pady=5, sticky="w")
        self.bias_check_button = ttk.Button(self.control_frame, text="현재 설정값")
        self.bias_check_button.configure(
            command=self._make_button_guarded_command(
        ...
        self.chamber_tmp_list = ["-20","0","20","40","60"]
        ...
        self.save_dir_label.grid(row=7, column=0, columnspan=4, padx=5, pady=5, sticky="we")

        self.histogram_button = ttk.Button(self.control_frame, text="히스토그램")
        self.histogram_button.configure(
            command=self._make_button_guarded_command(
    ...
    def _restore_button_when_ready(
        self,
        button: ttk.Button,
        in_progress_check=None,
        poll_ms: int = 200,
    ) -> None:
        if in_progress_check is None:
            self._set_button_enabled(button, True)
            return
        try:
            is_busy = bool(in_progress_check())
        except Exception:
            is_busy = False
        if is_busy:
            if self.root and self.root.winfo_exists():
                self.root.after(
                    poll_ms,
                    lambda: self._restore_button_when_ready(
                        button, in_progress_check=in_progress_check, poll_ms=poll_ms
                    ),
                )
            return
        self._set_button_enabled(button, True)

    def _make_button_guarded_command(self, button: ttk.Button, command, in_progress_check=None):
        def _handler():
            ...
            try:
                ...
                log_error(f"Button command failed: {exc}")
                traceback.print_exc()
            finally:
                self._restore_button_when_ready(button, in_progress_check=in_progress_check)
    ...
    def _ensure_histogram_window(self):
        ...
        x_axis = np.linspace(0, 16383, 512)
        (self.histogram_line,) = self.histogram_axis.plot(x_axis, np.zeros(512), linewidth=0.8)
    ...
    def _histogram_loop(self, cam_id: int) -> None:
        while self.histogram_running and self.histogram_cam_id == cam_id:
            ...
            with self.latest_frame_lock:
                hist = self.latest_histograms.get(cam_id)
                hist_copy = None if hist is None else hist.copy()
            if hist_copy is None:
                time.sleep(0.05)
                continue
            if not self._histogram_render_pending:
                self._histogram_render_pending = True
                self.root.after(0, self._render_histogram_canvas, cam_id, hist_copy)
            time.sleep(0.1)
    ...
    def _render_histogram_canvas(self, cam_id: int, hist: np.ndarray) -> None:
        ...
        if self.histogram_line is None or self.histogram_axis is None or self.histogram_canvas is None:
            return
        display_hist = hist.reshape(512, 32).sum(axis=1).astype(float)
        self.histogram_line.set_ydata(display_hist)
        y_max = max(1, int(display_hist.max() * 1.05))
    ...
    def create_camera_panel(self, row: int, column: int, cam_id: int) -> None:
        ...
        serial_connect_button.configure(
            command=self._make_button_guarded_command(
                serial_connect_button,
                lambda fid=cam_id: self.connect_serial_for_camera(fid),
            )
        )
        ...
        serial_disconnect_button.configure(
            command=self._make_button_guarded_command(
                serial_disconnect_button,
                lambda fid=cam_id: self.disconnect_serial_for_camera(fid),
            )
        )
    ...
    def detect_camera(self, cam_id):
        ...
        cap.set(cv2.CAP_PROP_FPS, self.target_camera_fps)
        self.camera[index] = cap
        actual_fps = cap.get(cv2.CAP_PROP_FPS)
        print(f"Camera {index} connected. target_fps={int(self.target_camera_fps)}, actual_fps={actual_fps:.2f}")
    ...
    def start_streaming(self, index):
        ...
        self._schedule_camera_update(index, delay_ms=1)

    def _schedule_camera_update(self, index: int, delay_ms: int = 1) -> None:
        if not self.camera_running.get(index):
            return
        job_id = self.root.after(max(1, int(delay_ms)), lambda idx=index: self.update_camera(idx))
        self.camera_after_jobs[index] = job_id

    def stop_streaming(self, cam_id):
        self.camera_running[cam_id] = False
        job_id = self.camera_after_jobs.pop(cam_id, None)
        if job_id:
            try:
        ...
        with self.latest_frame_lock:
            ...
            self.latest_histograms.pop(cam_id, None)
        if self.histogram_cam_id == cam_id:
            self._stop_histogram_thread()
    ...
    def update_camera(self, index):
        ...
        cap = self.camera.get(index)
        if cap is None or not cap.isOpened():
            self.camera_running.pop(index, None)
            self.camera_after_jobs.pop(index, None)
            return
        ...
        hist, _ = np.histogram(frame, bins=16384, range=(0, 16383))
        with self.latest_frame_lock:
            self.latest_histograms[index] = hist.copy()
        cdf = np.cumsum(hist)
        median_value = int(np.searchsorted(cdf, cdf[-1] / 2))
        widgets = self.camera_widgets.get(index)
        if widgets:
            widgets["hist_value"].config(text=f"{median_value}")
        elapsed = time.perf_counter() - loop_start
        delay_ms = max(1, int((self.camera_frame_interval - elapsed) * 1000))
        self._schedule_camera_update(index, delay_ms=delay_ms)
            ...
            if camera:
                ...
                for j in range(total_frames):
                    ...
                    if ret:
                        ...
                        conn = self.get_serial_connection(k)
                        fpa_raw = conn.get("fpa_raw") if conn else None
                        if fpa_raw is None:
                            fpa_raw = 0
                        frame[0,0] = int(fpa_raw)
                    ...
                    if bb == "20" or bb == "10":
                        with open(os.path.join(save_dir, camera_product, folder_name, "bias.json"), "w", encoding="utf-8") as f:
                            json.dump(self.bias_dict[k], f, indent=4, ensure_ascii=False)
    ...
    def get_fpa(self, frame_id: int = None):
        ...
        try:
            ...
            return conn.get("fpa_temp"), conn.get("fpa_raw")
        try:
    ...
    def auto_bias_setting(self, camera_num):
        """자동 바이어스 탐색 루틴."""
        if self.auto_bias_running:
            log_error("[AUTO BIAS] 이미 실행 중입니다. 완료 후 다시 시도하세요.")
            return
        ...
        if temp_range not in target_centers:
            log_error(f"[AUTO BIAS] 잘못된 온도 구간 선택: {temp_range}")
            return
        target = target_centers[temp_range]
        target_min, target_max = target - 100, target + 100
        self.auto_bias_running = True
        ...
        def _measure_median(frame_id: int, sample: int = 5):
            """update_camera가 저장한 최신 프레임으로 중앙값을 계산한다."""
            medians = []
            max_attempts = max(sample * 10, 20)
            for _ in range(max_attempts):
                with self.latest_frame_lock:
                    frame = self.latest_frames.get(frame_id)
                    frame_copy = None if frame is None else frame.copy()
                if frame_copy is None:
                    time.sleep(0.02)
                    continue
                avg = np.mean((frame_copy & 0x3FFF), dtype=np.int64)
                medians.append(float(avg))
                if len(medians) >= sample:
                    break
                time.sleep(0.02)
            if not medians:
                log_error(f"[AUTO BIAS] Camera {frame_id} 최신 프레임을 얻지 못했습니다.")
                return None
        ...
        def _worker():
            ...
            try:
                for cam_id in connected:
                    ...
                    if not success or gskL_value is None:
                    ...
                    print(f"[AUTO BIAS] Cam {cam_id} 보정 완료: CINT={cint_value}, TINT={tint_value}, GSK_MSB={gskM_value}, GSK_LSB={gsklv}, GFID={gfid_value}")
                self.auto_bias = True
